...
from ista_nas_single.search.models import NetworkCIFAR as Network_single
...
def main():
  ...
  if args.onestage:
    model = Network_single(args.init_channels, CIFAR_CLASSES, args.layers, args.auxiliary, genotype)
  else:
  ...
  train_transform, valid_transform = utils.cifar10_transforms(args)
  ...
  model1 = torch.jit.trace(model.to(device), x.cuda())
  torch.onnx.export(model1.to(device), # 搭建的网络
                x.cuda(), # 输入张量
